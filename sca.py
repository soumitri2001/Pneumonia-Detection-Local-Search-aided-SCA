...
def SCA(num_agents, max_iter, train_data, train_label, obj_function=compute_fitness, trans_func_shape='z', save_conv_graph=False):
            ...
            for j in range(num_features):
                ...
                if r4 < 0.5:
                    ...
                    population[i, j] = population[i, j] + \
                        (r1*np.sin(r2) *
                         abs(r3*Leader_agent[j]-population[i, j]))
                else:
                    # Eq. (3.2)
                    population[i, j] = population[i, j] + \
                        (r1*np.cos(r2) *
    ...
    Leader_agent, _, Leader_accuracy = sort_agents(
